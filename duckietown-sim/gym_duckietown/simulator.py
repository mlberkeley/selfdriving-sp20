...
class Simulator(gym.Env):
    ...
    def __init__(
        ...
        self.actions_as_voltages = actions_as_voltages
        self.kvel = kvel
        self.kvolt = kvolt
    ...
    def reset(self):
        ...
        self.colorless_tri_vlist = pyglet.graphics.vertex_list(3 * numTris, ('v3f', verts))

        self.white_tex = Texture(
            load_texture(os.path.join(os.path.dirname(__file__), 'textures/white.png'))
        )
        self.custom_semantic_texture_files = [
            '3way_left', '3way_right',
            '4way',
            'curve_left', 'curve_right',
            'straight'
        ]
        self.custom_map_texture_files = [
            '3way_left', '3way_right',
            '4way',
            'curve_left', 'curve_right',
            'straight'
        ]
        self.custom_semantic_textures = {}
        self.custom_map_textures = {}
        ...
        for tile in self.grid:
            ...
            tex = Texture.get(tile['kind'], rng=rng)
            tile['texture'] = tex
            if tile['kind'] in self.custom_semantic_texture_files:
                if not tex.variant in self.custom_semantic_textures:
                    self.custom_semantic_textures[tex.variant] = Texture(
                        load_texture(os.path.join(os.path.dirname(__file__), 'textures/semantic/'+tex.variant))
                    )
            if tile['kind'] in self.custom_map_texture_files:
                if not tex.variant in self.custom_map_textures:
                    self.custom_map_textures[tex.variant] = Texture(
                        load_texture(os.path.join(os.path.dirname(__file__), 'textures/map/'+tex.variant))
                    )
    ...
    def update_physics(self, action, delta_time=None):
        ...
        if self.actions_as_voltages:
            lastVels = self.wheelVels
            accelerations = self.kvolt * action + self.kvel * lastVels
            self.wheelVels = self.wheelVels + accelerations * delta_time
        else:
    ...
    def _render_img(self, width, height, multi_fbo, final_fbo, img_array, top_down=True):
        ...
        from pyglet import gl
        if self.semantic_mode or self.mapmode:
        ...
        if not top_down:
            ...
            gl.glOrtho(-2.5, 2.5, -2.5, 2.5, 0.04, 100)
        ...
        if self.semantic_mode or self.mapmode:
            self.colorless_tri_vlist.draw(gl.GL_TRIANGLES)
        else:
        ...
        for j in range(self.grid_height):
            for i in range(self.grid_width):
                ...
                if self.semantic_mode or self.mapmode:
                    ...
                    color = [1, 1, 1]
                    if self.semantic_mode and kind in self.custom_semantic_texture_files:
                        texture = self.custom_semantic_textures[texture.variant]
                    elif self.mapmode and kind in self.custom_map_texture_files:
                        texture = self.custom_map_textures[texture.variant]
                    else:
                        texture = self.white_tex

                        if self.mapmode:
                            color = [1, 0, 1] if tile['drivable'] else [1, 1, 0]
                        else:
                            color = [1, 1, 1] if tile['drivable'] else [1, 0, 0]
        ...
        if not self.mapmode:
            for idx, obj in enumerate(self.objects):
                obj.render(self.draw_bbox, self.semantic_mode)
    ...
    def render_obs(self, top_down=False):
        """
    ...
    def render(self, top_down=False, mode='human', close=False, filter=None):
        """
